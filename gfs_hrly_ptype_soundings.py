...
for i in range(0, 121, 24):
    ...
    h_contour = ax1.contour(x, y, mslpc, colors='dimgray',
                            levels=range(940, 1080, 4), linewidths=1, alpha=0.7)
    h_contour.clabel(fontsize=14, colors='dimgray', inline=1,
                     inline_spacing=4, fmt='%i mb', rightside_up=True, use_clabeltext=True)
    ...
    startlat = 78  # set the starting latitude of each row
    londelt = 5.85  # set the spacing between columns (data-wise, NOT visually!!)

    # make the soundings in rows from the bottom up. Bottom row starts at 1
    # to leave room for a legend in the lower right
    ...
    leg = ax1.legend(handles=[dashed_red_line, dashed_green_line, dashed_purple_line], title='Sounding Legend', loc=4,
                     framealpha=1)
    leg.set_zorder(100)
